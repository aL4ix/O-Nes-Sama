...
struct hash_time
{
    size_t operator()(const BreakpointPPUByTime &a ) const
    {
        return std::hash<int>()(a.scanline) ^ std::hash<int>()(a.tick);
    }
};

struct pred_time
{
    bool operator()(const BreakpointPPUByTime &a, const BreakpointPPUByTime &b) const
    {
        return a.scanline == b.scanline && b.tick == b.tick;
    }
};
#endif // DEBUGGER
...
public:
    PPU(struct InterruptLines &ints);
    ~PPU();
